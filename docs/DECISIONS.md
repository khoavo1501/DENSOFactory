...
## D-66 — plc-system topic namespace (M10)
- **Quyết định:** M10 gateway publish vào `plc-system/{master_id}/{status|telemetry}` (parallel với `devices/{id}/{...}` M1-M9).
- **Lý do:** Tách biệt convention. Gateway có model khác (Gateway + PLC + Assignment + Warning + Snapshot) so với thiết bị device. Source gateway PA15_MQTT_W5500 đã dùng convention này; simulator M1-M9 dùng `devices/+/+`.
- **Hệ quả:** Backend consumer subscribe cả 2 patterns; routing trong `_on_message` namespace để dispatch tới handler phù hợp.

## D-67 — PLC model 1:N future (M10)
- **Quyết định:** Model `PLC` có FK tới `Gateway`, model `PLCAssignment` mapping PLC → Gateway (1:1 hiện tại, 1:N future).
- **Lý do:** Firmware hiện tại PA15 có 1 PLC (nút PA15), nhưng gateway tương lai có thể có nhiều PLC. Schema đã sẵn sàng.
- **Hệ quả:** `GatewayWithPLCs.plcs` luôn là list; UI render folder theo số PLC.

## D-68 — Polling 1 phút mặc định, realtime on warning (M10)
- **Quyết định:** Frontend query interval là 60s cho PLC không warning; 5s cho PLC có warning active.
- **Lý do:** Giảm tải InfluxDB + backend. Khi có warning, cần realtime để operator theo dõi recovery; khi ổn định, polling 1 phút đủ.
- **Hệ quả:** Normal mode yêu cầu snapshot mỗi phút; realtime mode yêu cầu snapshot liên tục 5s. Toggling tự động dựa trên `activeWarnings` count.

## D-69 — Warning highlight CSS (M10)
- **Quyết định:** Vàng border + soft glow cho warning; đỏ border + glow mạnh hơn cho critical; left border 4px trên `PLCCard` + matching border color trên `GatewayCard`.
- **Lý do:** Operator cần nhìn ra ngay PLC/gateway nào có vấn đề khi scan grid 10-15 devices. Border trái 4px đủ mạnh, không làm rối layout.
- **Hệ quả:** Card offline mờ opacity 0.55; warning yellow 6% gradient; critical red 8% gradient. Pulse animation cho running state.

## D-70 — Unpaired devices explicit gate (M10)
- **Quyết định:** Gateway auto-create PLC khi nhận message đầu tiên (không qua assignment); `UnpairedSection` liệt kê PLC không có row trong `plc_assignments`.
- **Lý do:** Không muốn block pipeline: message publish vẫn được nhận + warning vẫn raise; UI cho admin pair về gateway để "quản lý" (group/display). Tách concerns: ingestion (luôn OK) vs management (cần opt-in).
- **Hệ quả:** Operator thấy danh sách PLC mới trong `/unpaired`, pair bằng 1 click; warning vẫn hiển thị kể cả PLC chưa pair.
